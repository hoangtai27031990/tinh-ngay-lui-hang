...
const SW_VERSION = "hsd-sw-v4";
const CACHE_NAME = "hsd-sw-meta-v4";
...
self.addEventListener("activate", e  => e.waitUntil(
  /* Xóa cache cũ của v3 khi nâng cấp */
  caches.delete("hsd-sw-meta-v3").then(() => self.clients.claim())
));
...
function doCheckAndNotify(products) {
  ...
  if (itemsExp.length) {
    ...
    fire("❌ Sản phẩm HẾT HẠN – Co.op",
      `${itemsExp.length} SP hết hạn: ${fmt(names)}`, "exp", itemsExp);
  }
  if (items20.length) {
    const names = items20.map(i => i.TenSP || ("BC: " + (i.Barcode || "?")));
    fire("🔴 RÚT HÀNG ngay – Co.op",
      `${items20.length} SP qua mốc 20%: ${fmt(names)}`, "20", items20);
  }
  if (items30.length) {
    const names = items30.map(i => i.TenSP || ("BC: " + (i.Barcode || "?")));
    fire("⚠️ GIẢI TỒN – Co.op",
      `${items30.length} SP đến mốc 30%: ${fmt(names)}`, "30", items30);
  }
}
...
/* ================================================
   6. CLICK VÀO THÔNG BÁO → MỞ APP + HIỂN THỊ SẢN PHẨM
   ================================================ */
